...
**Interaction feel:** swipe a card right to complete (a green "trail" with a check icon is revealed behind it as it slides), left to delete (red trail, trash icon). Past 16.18% of screen width it slides off in 240ms; the list closes the gap with a soft spring; a card returning to the list glides in from the side over 647ms with NO bounce — deliberate, unhurried. An undo toast (dark rounded bar, bottom of screen) appears after destructive actions. A single round **FAB** (56pt, indigo `#4954C7` light / `#7C8BC4` dark) floats bottom-right for quick-add.

**Newer surfaces you can also style (added 2026-07-16→23):** the day page's TIMELINE (hour ruler in the mono font, events as duration-sized blocks with a 3px colored left bar, tasks as uniform 56pt rows with a status bar and a tap-to-complete circle, a red "now" line); the month calendar's named bars (status-tinted task bars, dashed-border event bars whose light-mode text is auto-darkened to ≥7:1 — you cannot lower that) and tiny count numbers beside the dots; the iOS home/lock widgets (status accent bars, mono due labels, check circles, calendar-glyph event rows — lock-screen ones render monochrome by OS rule); the CSV import sheet's Events/Tasks segmented selector; and the sync-state dot (blue = live, accent = syncing, red OUTLINED ring = offline — the outline encoding is locked). Auth screens accept a background only.
