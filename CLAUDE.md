...
- **Next milestone:** verify debug CRUD on phone → then the first real feature slice: design tokens + the designed task list/Today view (per docs/design/), or straight to Stage 3 offline sync — decide with developer. Offline sync engine (PowerSync vs WatermelonDB) still open.
