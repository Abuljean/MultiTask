...
- **UI slice 4 built (2026-07-10): quick-add v1.** `components/fab.tsx` (accent FAB, bottom-right, the screen's one primary action) + `components/quick-add-sheet.tsx` (Modal bottom sheet: autofocus title, date chip → inline calendar picker, time chip → 15-min wheel, both in JetBrains Mono; Enter or "Add task" submits; haptic success; new card slides into its sorted spot via the entrance-mark system). Uses `@react-native-community/datetimepicker` (works in Expo Go). Defaults: due = next quarter-hour today. "+ details" reveal (category/subject/priority/description) deliberately deferred to the task-detail slice. **Pending: on-phone review (incl. FAB-tap-to-keyboard latency — target <200ms).**
- **Next slices (in order):** Today view → bulk "clear completed" → task detail/edit view (description + "+ details" fields live there) → calendar.
