...
- **Payments — DECIDED (2026-07-11): sales happen on the WEBSITE, not through Apple IAP.** Packs are bought on the web (Stripe, ~3% fees), entitlement recorded against the buyer's Supabase account, and the app unlocks owned packs on any device. The in-app marketplace page is a gallery that links out to the web checkout — permitted in the US post-Epic-injunction (2024–25 rulings allow external purchase links without Apple commission) and always permitted as the "purchased elsewhere, consumed in-app" model. CAVEATS to re-verify at ship time: rules differ by region and keep shifting (EU DMA fine; some regions still restrict link-outs — degrade to gallery-without-link there); Apple review may scrutinize wording; a Stripe account itself requires age of majority (guardian-owned initially). Creator payouts (phase 2) still need Stripe Connect + tax handling — simpler on web, still real work.
